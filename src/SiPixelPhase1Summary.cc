...
SiPixelPhase1Summary::SiPixelPhase1Summary(const edm::ParameterSet& iConfig)
  ...
  //modified by sayan on 18.16.2021
...
void SiPixelPhase1Summary::fillSummaries(DQMStore::IBooker& iBooker, DQMStore::IGetter& iGetter) {
  ...
  for (unsigned int i = 0; i < trendOrder.size(); i++) {
    ...
    //modified by sayan on 18.06.2021
 ...
 //    continue;
	  ...
	  { 
	    ...
	    //*Warning*
	    if (deadRocWarnThresholds_[i*4 + j] < deadROCSummary->getBinContent(i + 1, j + 1) && deadROCSummary->getBinContent(i + 1, j + 1) < deadRocThresholds_[i * 4 + j])
